...
public class ProClientesService {
    ...
    public Respuesta recuperarClave(String correo) {
        try {
            Query qryActividad = em.createNamedQuery("ProClientes.findByCliCorreo", ProClientes.class);
            qryActividad.setParameter("cliCorreo", correo);
            ProClientesDto proClientesDto = new ProClientesDto((ProClientes) qryActividad.getSingleResult());
            recuClave(proClientesDto);
            ProClientes proClientes;
            if (proClientesDto.getCliId() != null && proClientesDto.getCliId() > 0) {
                proClientes = em.find(ProClientes.class, proClientesDto.getCliId());
                if (proClientes == null) {
    ...
    public Respuesta recuClave(ProClientesDto proClientesDto){
       int len = 8;
        ...
        try {
            ...
            proClientesDto.setCliClave(claveRestaurada);
            proClientesDto.setCliClaverestaurada(claveRestaurada);
            return new Respuesta(true, CodigoRespuesta.CORRECTO, "", "","");
